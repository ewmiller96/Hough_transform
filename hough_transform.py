...
# these functions load events and return their hit info in pandas data frames, labels_2 and labels_3 re for rare cases of hits shared by tracks

def load_single_mu(event):
    hits = pd.read_csv("mu10GeV/clusters_"+str(event)+".CSV", names = ["x", "y", "z", "labels", "labels_2", "labels_3", "track"])
    truth = pd.read_csv("mu10GeV/truth_"+str(event)+".CSV", names = ["labels", "pt", "eta", "phi", "e"])
    hits.fillna(0.0, inplace=True)
    truth.fillna(0.0, inplace=True)
    return hits, truth

def load_four_mu(event):
    hits = pd.read_csv("mu10GeV_4particles/clusters_"+str(event)+".CSV", names = ["x", "y", "z", "labels", "labels_2", "labels_3", "track"])
    truth = pd.read_csv("mu10GeV_4particles/truth_"+str(event)+".CSV", names = ["labels", "pt", "eta", "phi", "e"])
    hits.fillna(0.0, inplace=True)
    truth.fillna(0.0, inplace=True)
    return hits, truth

def load_nine_mu(event):
    hits = pd.read_csv("mu10GeV_9particles/clusters_"+str(event)+".CSV", names = ["x", "y", "z", "labels", "labels_2", "labels_3", "track"])
    truth = pd.read_csv("mu10GeV_9particles/truth_"+str(event)+".CSV", names = ["labels", "pt", "eta", "phi", "e"])
    hits.fillna(0.0, inplace=True)
    truth.fillna(0.0, inplace=True)
    return hits, truth

def load_ttbar(event):
    hits = pd.read_csv("ttbar/clusters_277"+"{:0>4d}".format(event)+".CSV", names = ["x", "y", "z", "labels", "labels_2", "labels_3", "track"])
    truth = pd.read_csv("ttbar/truth_277"+"{:0>4d}".format(event)+".CSV", names = ["labels", "pt", "eta", "phi", "e"])
    hits.fillna(0.0, inplace=True)
    truth.fillna(0.0, inplace=True)
    return hits, truth
...
event = 5 #event to load, between 1 and 5
data, truth = load_foursss_mu(event) #change to load different no. of muons or ttbar event
